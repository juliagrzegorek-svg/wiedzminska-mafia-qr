...
/**
 * Prosta karta z „pociągnięciem rogu”.
 * Props:
 *  - title, subtitle, imageUrl, description
 *  - onPlace?: () => void (klik „Odłóż kartę na stół”)
 */
export default function PeelCard({ title, subtitle, imageUrl, description, onPlace }) {
...
export default function PeelCard({ title, subtitle, imageUrl, description, onOpen, onClose }) {
  ...
  const doOpen = () => { if (!open) { setOpen(true); onOpen && onOpen(); } };
  const doClose = () => { if (open) { setOpen(false); onClose && onClose(); } };
