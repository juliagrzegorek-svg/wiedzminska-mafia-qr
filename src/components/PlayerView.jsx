...
export default function PlayerView() {
  ...
  if (!payload || payload.t !== "player") {
    return (
      <div className="mx-auto mt-10 max-w-xl rounded-2xl border border-zinc-700/50 bg-zinc-900/70 p-5 text-center text-zinc-200">
        ...
        <p className="mt-2 text-sm text-zinc-400">Ten link nie zawiera informacji o graczu. Poproś gospodarza o nowy link lub wróć na stronę główną.</p>
        <a href="/" className="mt-4 inline-block rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm">Wróć na stronę główną</a>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 text-zinc-200">
      <div className="mb-4 text-sm text-zinc-400">ID gry: <span className="font-mono">{payload.gid}</span></div>
      <h1 className="text-2xl font-bold">Twoje karty, {payload.name}:</h1>

      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        {hero && <PeelCard title={hero.name} subtitle="Bohater" imageUrl={hero.image} description="" />}
        {monster && <PeelCard title={monster.name} subtitle="Potwór" imageUrl={monster.image} description={monster.description} />}
      </div>
