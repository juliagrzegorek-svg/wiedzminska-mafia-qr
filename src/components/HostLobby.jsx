...
export default function HostLobby() {
  ...
  const deal = () => {
    ...
    // rozdaj bohaterów losowo (z rotacją, jeśli graczy > bohaterów)
    ...
    // wybierz graczy na potwory
    const mCount = Math.min(monstersCount, assigned.length);
    const chosenIdx = shuffle(assigned.map((_, i) => i)).slice(0, mCount);

    // rozdaj typy potworów (z powtórkami, jeśli braknie)
    const chosenTypes = [];
    for (let i = 0; i < mCount; i++) chosenTypes.push(MONSTERS[i % MONSTERS.length]);

    chosenIdx.forEach((i, k) => {
      assigned[i].monster = chosenTypes[k];
    });

    // wygeneruj prywatne linki/QR
  ...
  return (
    <section className="mx-auto max-w-5xl rounded-2xl border border-zinc-700/50 bg-zinc-900/70 p-4">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="basis-2/3">
          ...
          <ul className="mt-3 grid grid-cols-2 gap-2">
            {players.map((p) => (
              ...
              >
                <span>{p.name}</span>
                <button className="text-zinc-400 hover:text-red-400" onClick={() => removePlayer(p.id)}>
                  Usuń
                </button>
              </li>
            ))}
        ...
        <div className="basis-1/3 rounded-xl border border-zinc-700/60 bg-zinc-950 p-3">
          ...
          <ol className="mt-2 list-inside list-decimal text-sm text-zinc-300">
            ...
            <li>
              Każdemu graczowi wyślij jego <em>Link</em> lub pokaż <em>QR</em>.
            </li>
          </ol>
        </div>
      </div>
      ...
      {dealt && (
        <div className="mt-5">
          <div className="mb-2 text-sm font-semibold text-zinc-200">Rozdane karty (tajne linki):</div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {dealt.map((row) => {
                  const abilityTitle = abilityById[row.hero.baseAbilityId]?.title || "—";
                  return (
                    <tr key={row.name} className="border-t border-zinc-800">
                      ...
                      <td className="pr-3 text-zinc-300">{abilityTitle}</td>
