...
export default function App(){
  ...
  const [hero,setHero] = useState(()=>{
    const raw = localStorage.getItem(LS.hero); return raw? JSON.parse(raw):null
  })
  const [monster,setMonster] = useState(()=>{
    const raw = localStorage.getItem(LS.monster); return raw? JSON.parse(raw):null
  })
  const [ability,setAbility] = useState(()=>{
    const raw = localStorage.getItem(LS.ability); return raw? JSON.parse(raw):null
  })
  ...
  function onHeroClick(){
    if(step==='hero'){
      ...
      setFocus('left')
      if(monster){
        setTimeout(()=> setStep('monster'), 450)
      } else {
  ...
  function onOverlayClick(){
    ...
    if(hero && !ability){
      ...
      const result = { ...good, ownerName: hero.name }
      setAbility(result)
    }
    if(monster && !ability){
      const ma = randItem(MONSTER_ABILITIES)
      const result = { ...ma, ownerName: monster.name }
      setAbility(result)
    }
